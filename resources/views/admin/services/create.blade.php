            ...
            <form action="{{ route('admin.services.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('icon') is-invalid @enderror" id="icon" name="icon"
                        value="{{ old('icon', 'bi-activity') }}" required>
                    <div class="form-text">Example: <code>bi-heart-pulse</code>, <code>bi-capsule</code>. See <a
                            href="https://icons.getbootstrap.com/" target="_blank">Bootstrap Icons</a>.</div>
                    @error('icon')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
